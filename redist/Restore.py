...
# greyscale image produces message
# iteration count corrected and default colour balance values changed
# 31/3/10
# minor corrections to make comaptable with python 2.6
# 4/12/11
# improved iteration and error messages as opposed to crash
#
from gimpfu import *
from math import *
...
# message on attempt to restore greyscale image
    if pdb.gimp_drawable_is_gray(drawable):
        pdb.gimp_message("Restore does not work on greyscale images.  Either use the levels command from the colors menu, or scan the image in colour (not convert from greyscale to RGB), restore it and then convert the result to greyscale.")
        return
...
#        debugfile.write("during iter"+str(iter)+str(alpha)+"\n")
        for C in [R,G,B]:
            if (alpha[C]<0.1) or (alpha[C]>10.0):
                pdb.gimp_message("The image has deteriorated too far to restore.  You may be able to improve it by using the 'levels' operation first to improve the colour and then trying 'Restore'")
                return
...
#        debugfile.write("alpha iteration failed to converge\n")
        pdb.gimp_message("The image has deteriorated badly; the restoration is probably poor but may be an improvement on the original.")
        alpha=initalpha[:]; m=initm[:]
