...
Octree::Octree<T,AS>::Octree( int size, const T& emptyValue )
    : root_(0)
 ...
 */
template< typename T, int AS >
Octree::Octree<T,AS>::Octree( const Octree<T,AS>& o )
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::swap( Octree<T,AS>& o )
{
...
template< typename T, int AS >
Octree::Octree<T,AS>&
Octree::Octree<T,AS>::operator= ( Octree<T,AS> o )
{
    swap(o);
    return *this;
}
...
template< typename T, int AS >
const T&
Octree::Octree<T,AS>::emptyValue() const
{
    return emptyValue_;
}
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::setEmptyValue( const T& emptyValue )
{
    emptyValue_ = emptyValue;
}
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::deleteNode( Node** node )
{
    assert(node);
    if (*node) {
 ...
 */
template< typename T, int AS >
typename Octree::Octree<T,AS>::Node*&
Octree::Octree<T,AS>::root()
{
    return root_;
}
 ...
 */
template< typename T, int AS >
const typename Octree::Octree<T,AS>::Node*
Octree::Octree<T,AS>::root() const
{
    return root_;
}
...
template< typename T, int AS >
const T&
Octree::Octree<T,AS>::at( int x, int y, int z ) const
{
    assert( x >= 0 && x < size_ );
    assert( y >= 0 && y < size_ );
    assert( z >= 0 && z < size_ );
...
template< typename T, int AS >
const T&
Octree::Octree<T,AS>::operator() ( int x, int y, int z ) const
{
    return at(x,y,z);
}
...
template< typename T, int AS >
T&
Octree::Octree<T,AS>::operator() ( int x, int y, int z )
{
    assert( x >= 0 && x < size_ );
    assert( y >= 0 && y < size_ );
    assert( z >= 0 && z < size_ );
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::set( int x, int y, int z, const T& value )
{
    if ( value != emptyValue() ) {
        (*this)(x,y,z) = value;
    }
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::erase( int x, int y, int z )
{
    assert( x >= 0 && x < size_ );
    assert( y >= 0 && y < size_ );
    assert( z >= 0 && z < size_ );
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::eraseRecursive( Node** node, int size, int x, int y, int z )
{
    assert(node);
...
template< typename T, int AS >
unsigned long
Octree::Octree<T,AS>::branchBytes()
{
...
template< typename T, int AS >
unsigned long
Octree::Octree<T,AS>::aggregateBytes()
{
...
template< typename T, int AS >
int
Octree::Octree<T,AS>::nodes() const
{
...
template< typename T, int AS >
int
Octree::Octree<T,AS>::nodesRecursive( const Node* node )
{
    if ( !node ) {
        return 0;
    }
...
template< typename T, int AS >
unsigned long
Octree::Octree<T,AS>::bytes() const
{
...
template< typename T, int AS >
unsigned long
Octree::Octree<T,AS>::bytesRecursive( const Node* node )
{
    if ( !node ) {
        return 0;
    }
...
template< typename T, int AS >
int
Octree::Octree<T,AS>::nodesAtSize( int size ) const
{
...
template< typename T, int AS >
int
Octree::Octree<T,AS>::nodesAtSizeRecursive( int targetSize, int size, Node* node )
{
    if (node) {
        if ( size == targetSize ) {
            return 1;
...
template< typename T, int AS >
Octree::Octree<T,AS>::Node::Node( NodeType type )
    : type_(type)
{
}
...
template< typename T, int AS >
typename
Octree::Octree<T,AS>::NodeType
Octree::Octree<T,AS>::Node::type() const
{
    return type_;
}

template< typename T, int AS >
Octree::Octree<T,AS>::Branch::Branch()
    : Node(BranchNode)
{
...
template< typename T, int AS >
Octree::Octree<T,AS>::Branch::Branch( const Branch& b )
    : Node(BranchNode)
{
    for ( int i = 0; i < 8; ++i ) {
        if ( b.child(i) ) {
...
template< typename T, int AS >
const typename Octree::Octree<T,AS>::Node*
Octree::Octree<T,AS>::Branch::child( int x, int y, int z ) const
{
    assert( x == 0 || x == 1 );
    assert( y == 0 || y == 1 );
    assert( z == 0 || z == 1 );
...
template< typename T, int AS >
typename
Octree::Octree<T,AS>::Node*&
Octree::Octree<T,AS>::Branch::child( int x, int y, int z )
{
    assert( x == 0 || x == 1 );
    assert( y == 0 || y == 1 );
    assert( z == 0 || z == 1 );
...
template< typename T, int AS >
const typename Octree::Octree<T,AS>::Node*
Octree::Octree<T,AS>::Branch::child( int index )const
{
...
typename
Octree::Octree<T,AS>::Node*&
Octree::Octree<T,AS>::Branch::child( int index )
{
    assert( index >= 0 && index < 8 );
    return *( &children[0][0][0] + index );
}
...
template< typename T, int AS >
Octree::Octree<T,AS>::Aggregate::Aggregate( const T& v )
    : Node(AggregateNode)
{
    for ( int i = 0; i < AS; ++i ) {
        for ( int j = 0; j < AS; ++j ) {
...
template< typename T, int AS >
const T&
Octree::Octree<T,AS>::Aggregate::value( int x, int y, int z ) const
{
    assert( x >= 0 && x < AS );
    assert( y >= 0 && y < AS );
    assert( z >= 0 && z < AS );
...
template< typename T, int AS >
T& Octree::Octree<T,AS>::Aggregate::value( int x, int y, int z )
{
    assert( x >= 0 && x < AS );
    assert( y >= 0 && y < AS );
    assert( z >= 0 && z < AS );
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::Aggregate::setValue( int x, int y, int z, const T& v )
{
    assert( x >= 0 && x < AS );
    assert( y >= 0 && y < AS );
    assert( z >= 0 && z < AS );
...
template< typename T, int AS >
const T& Octree::Octree<T,AS>::Aggregate::value( int i ) const
{
...
template< typename T, int AS >
T& Octree::Octree<T,AS>::Aggregate::value( int i )
{
    assert( i >= 0 && i < AS*AS*AS );
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::Aggregate::setValue( int i, const T& v )
{
    assert( i >= 0 && i < AS*AS*AS );
...
template< typename T, int AS >
Octree::Octree<T,AS>::Leaf::Leaf( const T& v )
    : Node(LeafNode)
    , value_(v)
{
}
...
template< typename T, int AS >
const T&
Octree::Octree<T,AS>::Leaf::value() const
{
    return value_;
}
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::Leaf::setValue( const T& v )
{
    value_ = v;
}
...
template< typename T, int AS >
Octree::Array2D<T>
Octree::Octree<T,AS>::zSlice( int z ) const
{
    assert( z >= 0 && z < size_ );
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::zSliceRecursive( Array2D<T> slice, const Node* node,
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::writeBinary( std::ostream& out ) const
{
    if ( !root_ ) {
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::writeBinaryRecursive( std::ostream& out, const Node* node )
{
    assert(node);
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::readBinary( std::istream& in )
{
    Octree<T,AS> tmp(0);
...
template< typename T, int AS >
void
Octree::Octree<T,AS>::readBinaryRecursive( std::istream& in, Node** node )
{
    assert(node);
